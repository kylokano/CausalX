...
from recbole.sampler import KGSampler, Sampler, RepeatableSampler, ReasonsSampler
...
def get_dataloader(config, phase: Literal["train", "valid", "test", "evaluation"]):
    ...
    model_type = config["MODEL_TYPE"]
    if phase == "train":
        if "reasons_sampler" in config and config["reasons_sampler"]:
            return ReasonsTrainDataLoader
        if model_type != ModelType.KNOWLEDGE:
        ...
        eval_limited = config["eval_args"]["limited"]["is_limited"]
        if eval_limited and eval_mode == "full":
            raise ValueError("cannot set evallimited and full mode together")
        if eval_mode == "full":
            if "reasons_sampler" in config and config["reasons_sampler"]:
                return FullSortReasonsEvalDataLoader
            else:
                return FullSortEvalDataLoader
        else:
            if eval_limited:
                return LimitedNumberNegSampleEvalDataLoader
            else:
