...
class ReasonsSampler(AbstractSampler):
    """:class:`RepeatableSampler` is used to sample negative items for each input user. The difference from
    :class:`Sampler` is it can only sampling the items that have not appeared at all phases.
    ...
    def __init__(self, phases, dataset, distribution="uniform", alpha=1.0):
        ...
        if distribution != "uniform":
            raise KeyError("ReasonsSampler only support uniform distribution")
        
        self.uid_field = dataset.uid_field
        self.iid_field = dataset.iid_field
        self.rid_filed = "reasons_id"
        
        self.user_num = dataset.user_num
        self.item_num = dataset.item_num
        
        self.reasons_list = set(["[PAD]"])
        self.ui_pair_reasons = defaultdict(set)
        self.used_id = np.array([set() for _ in range(self.user_num)])
        self.item_used_id = np.array([set() for _ in range(self.item_num)])
        
        for uid, iid, rid in zip(
            dataset.inter_feat[self.uid_field].numpy(),
            dataset.inter_feat[self.iid_field].numpy(),
            dataset.inter_feat[self.rid_filed].numpy(),
        ):
            self.used_id[uid].add(iid)
            self.item_used_id[iid].add(uid)
            self.ui_pair_reasons[str(uid)+";"+str(iid)].add(rid)
            self.reasons_list.add(rid)
        self.reasons_num = len(self.reasons_list)
        self.ui_pair_num = len(self.ui_pair_reasons)+1
       
        self.reasons_used_id = [set()]
        ui_pairs_idx = 1
        self.ui_pair2idx = {}
        for ui_key, reasons_set in self.ui_pair_reasons.items():
            self.reasons_used_id.append(reasons_set)
            self.ui_pair2idx[ui_key] = ui_pairs_idx
            ui_pairs_idx += 1
        assert ui_pairs_idx == len(self.reasons_used_id)
        self.reasons_used_id = np.array(self.reasons_used_id)
    ...
    def _uni_reasons_sampling(self, sample_num):
        return np.random.randint(1, self.reasons_num, sample_num)
    
    def _uni_user_sampling(self, sample_num):
        return np.random.randint(1, self.user_num, sample_num)

    def _get_candidates_list(self):
        return list(self.dataset.inter_feat[self.iid_field].numpy())
    
    def _get_reasons_candidates_list(self):
        return list(self.reasons_list)
    
    def _get_user_candidates_list(self):
        raise NotImplementedError("Current not support reasons trainging with pop negsample")
    ...
    def get_reasons_used_id(self):
        return self.reasons_used_id
    
    def get_item_used_ids(self):
        return self.item_id
    
    def sample_user_by_item_ids(self, user_ids, item_ids, num):
        try:
            # self.used_ids = np.array([{i} for i in item_ids])
            return self.sample_user_by_key_ids(item_ids, num)
        except IndexError:
            for item_id in item_ids:
                if item_id < 0 or item_id >= self.item_num:
                    raise ValueError(f"user_id [{item_id}] not exist.")
    ...
    def sample_reasons_by_user_ids(self, user_ids, item_ids, reasons_ids, num):
        """Sampling by user_ids.

        Args:
            user_ids (numpy.ndarray or list): Input user_ids.
            item_ids (numpy.ndarray or list): Input item_ids.
            reasons_ids (numpy.ndarray or list): Input item_ids.
            num (int): Number of sampled item_ids for each user_id.
        ...
        try:
            ...
            for uid, iid in zip(user_ids, item_ids):
                ui_pair_ids.append(self.ui_pair2idx[str(uid)+";"+str(iid)])
            ui_pair_ids = np.array(ui_pair_ids)
            return self.sample_reasons_by_key_ids(ui_pair_ids, num)
        except IndexError:
            for user_id in user_ids:
                if user_id < 0 or user_id >= self.user_num:
                    raise ValueError(f"user_id [{user_id}] not exist.")
    ...
    def sample_user_by_key_ids(self, key_ids, num):
        """Sampling by key_ids.
        ...
        if (key_ids == key_ids[0]).all():
            key_id = key_ids[0]
            used = np.array(list(self.item_used_id[key_id]))
            value_ids = self.neg_user_sampling(total_num)
            check_list = np.arange(total_num)[np.isin(value_ids, used)]
            while len(check_list) > 0:
                value_ids[check_list] = value = self.neg_user_sampling(len(check_list))
                mask = np.isin(value, used)
                check_list = check_list[mask]
        else:
            value_ids = np.zeros(total_num, dtype=np.int64)
            ...
            key_ids = np.tile(key_ids, num)
            while len(check_list) > 0:
                value_ids[check_list] = self.neg_user_sampling(len(check_list))
                check_list = np.array(
                    [
                        i
                        for i, used, v in zip(
                            check_list,
                            self.item_used_id[key_ids[check_list]],
                            value_ids[check_list],
                        )
                        if v in used
                    ]
    ...
    def sample_reasons_by_key_ids(self, key_ids, num):
        """Sampling by key_ids.
        ...
        if (key_ids == key_ids[0]).all():
            key_id = key_ids[0]
            used = np.array(list(self.reasons_used_id[key_id]))
            value_ids = self.reasons_sampling(total_num)
            check_list = np.arange(total_num)[np.isin(value_ids, used)]
            while len(check_list) > 0:
                value_ids[check_list] = value = self.reasons_sampling(len(check_list))
                mask = np.isin(value, used)
                check_list = check_list[mask]
        else:
            value_ids = np.zeros(total_num, dtype=np.int64)
            ...
            key_ids = np.tile(key_ids, num)
            while len(check_list) > 0:
                value_ids[check_list] = self.reasons_sampling(len(check_list))
                check_list = np.array(
                    [
                        i
                        for i, used, v in zip(
                            check_list,
                            self.reasons_used_id[key_ids[check_list]],
                            value_ids[check_list],
                        )
                        if v in used
                    ]
    ...
    def reasons_sampling(self, sample_num):
        """Sampling [sample_num] item_ids.
        ...
        """
        if self.distribution == "uniform":
            return self._uni_reasons_sampling(sample_num)
        # elif self.distribution == "popularity":
        #     return self._pop_sampling(sample_num)
        else:
            raise NotImplementedError(
    ...
    def neg_user_sampling(self, sample_num):
        """Sampling [sample_num] item_ids.
        ...
        """
        if self.distribution == "uniform":
            return self._uni_user_sampling(sample_num)
        # elif self.distribution == "popularity":
        #     return self._pop_sampling(sample_num)
        else:
            raise NotImplementedError(
