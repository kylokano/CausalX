...
from recbole.data.dataloader import FullSortEvalDataLoader, NegSampleEvalDataLoader, FullSortReasonsEvalDataLoader
...
class Trainer(AbstractTrainer):
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            if "reasons_sampler" in self.config and self.config["reasons_sampler"]:
                interaction = {k:v.to(self.device) for k,v in interaction.items()}
            else:
    ...
    def _full_sort_batch_eval(self, batched_data):
        ...
        try:
            ...
            if "reasons_sampler" in self.config and self.config["reasons_sampler"]:
                ...
                interaction = [i.to(self.device) for i in interaction]
    ...
    def _LLM_batch_eval(self, batched_data):
        """eval_LLM_ranker
        Args:
            batched_data (list): pred_u, pred_i, pred_score, positive_u, positive_i, item_number

        Returns:
            _, scores, positive_u, positive_i
        """
        if self.config["eval_type"] != EvaluatorType.RANKING:
            raise KeyError("LLM only supports ranking evaluation")
        if len(batched_data) == 4: # baselines
            interaction, row_idx, positive_u, positive_i = batched_data
            batch_size = interaction.length
            if batch_size <= self.test_batch_size:
                origin_scores = self.model.predict(interaction.to(self.device))
            ...
            pred_u, pred_i, pred_score, positive_u, positive_i = batched_data
            batch_user_num = positive_u[-1] + 1
            scores = torch.full(
                    (batch_user_num, self.tot_item_num), -np.inf, device=self.device
                )
            scores[pred_u, pred_i] = torch.tensor(pred_score, device=scores.device, dtype=scores.dtype)
            return None, scores, positive_u, positive_i
    ...
    def evaluate(
        ...
        if load_best_model is not None:
            ...
            self.model.eval()
        # added code to make fair comperation
        if type(eval_data) == list and eval_data[0] == "baseline":
            self.model.change_for_modelid(eval_data[1], eval_data[2]) # user_model_id2baseline_id, item_model_id2baseline_id
            eval_data = eval_data[3]
        
        if isinstance(eval_data, FullSortEvalDataLoader) or isinstance(eval_data, FullSortReasonsEvalDataLoader):
            eval_func = self._full_sort_batch_eval
            if self.item_tensor is None:
                self.item_tensor = eval_data._dataset.get_item_feature().to(self.device)
        elif isinstance(eval_data, NegSampleEvalDataLoader):
            eval_func = self._neg_sample_batch_eval
        else:
            # modified for LLM
            eval_func = self._LLM_batch_eval
        if self.config["eval_type"] == EvaluatorType.RANKING:
            if isinstance(eval_data, list):
                self.tot_item_num = eval_data[-1]
                eval_data = [eval_data[:-1]]
            else:
                if "reasons_sampler" in self.config and self.config["reasons_sampler"]:
                    self.tot_item_num = eval_data.reasons_num
                else:
