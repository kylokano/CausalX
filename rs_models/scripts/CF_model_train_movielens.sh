                ...
                for ir in "${ir_u_loss[@]}"; do
                    for uir in "${u_ir_loss[@]}"; do
                        ...
                        # Command to run the Python script
                            python CF_model_train.py --gpu_id=1 --dataset=cluster_results_MiniLM_UMAP20_openai_movielens --learning_rate=$lr --is_pairwise=False --ui_ratio=$ui_r --reason_ratio=$reason_ratio --neg_user_loss=$neg --ur_i_loss=$ur --ir_u_loss=$ir --u_ir_loss=$uir --reg_loss=$rgl --wandb_project=new_reason_CF
                        done
                    done
                done
            done
